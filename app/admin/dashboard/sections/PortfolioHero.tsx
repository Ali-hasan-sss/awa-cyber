...
export default function PortfolioHeroSection() {
  ...
  const loadHeroSection = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getSections({ page: "portfolio" });
    ...
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">
          {isArabic ? "هيرو صفحة معرض الأعمال" : "Portfolio Page Hero"}
        </h2>
        {!isEditing ? (
          <Button
      ...
      <div className="relative bg-white rounded-3xl border border-white/10 p-8 md:p-12">
        {isEditing ? (
          ...
          /* View Mode (matches client/components/portfolio/PortfolioHero.tsx) */
