...
import { Pencil, Save, X, Trash2, Plus, ChevronUp, ChevronDown, CheckCircle2 } from "lucide-react";
...
export default function SecurityModalSection() {
  const { locale } = useLanguage();
  const isArabic = locale === "ar";
  const [securityModalSection, setSecurityModalSection] =
  ...
  useEffect(() => {
    loadSecurityModalSection();
  }, []);

  const loadSecurityModalSection = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getSections({ page: "home" });
      // Get the ninth section (index 8) sorted by order
      const sortedData = data.sort((a, b) => a.order - b.order);
      if (sortedData && sortedData.length > 8) {
        const section = sortedData[8];
        setSecurityModalSection(section);
        setForm({
      ...
      setError(err.message || "Failed to load Security Modal section");
      console.error("Error loading Security Modal section:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!securityModalSection) return;
    ...
    try {
      ...
      await updateSection(securityModalSection._id, payload);
      await loadSecurityModalSection();
      setIsEditing(false);
    } catch (err: any) {
      setError(err.message || "Failed to save Security Modal section");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCancel = () => {
    if (securityModalSection) {
      setForm({
        titleEn: securityModalSection.title?.en || "",
        titleAr: securityModalSection.title?.ar || "",
        descriptionEn: securityModalSection.description?.en || "",
        descriptionAr: securityModalSection.description?.ar || "",
        features:
          securityModalSection.features &&
          securityModalSection.features.length > 0
            ? securityModalSection.features.map((feature) => ({
                icon: feature.icon || "",
                nameEn: feature.name?.en || "",
                nameAr: feature.name?.ar || "",
                descriptionEn: feature.description?.en || "",
      ...
      setActiveFeatureIndex(
        securityModalSection.features && securityModalSection.features.length > 0
          ? 0
          : null
      );
    }
  ...
  const addFeature = () => {
    // Limit to 3 features only
    if (form.features.length >= 3) {
      setError(isArabic ? "الحد الأقصى 3 ميزات" : "Maximum 3 features allowed");
      return;
    }
  ...
  const sectionTitle = isEditing
    ...
    : securityModalSection?.title?.[locale] ||
      (locale === "ar" ? "جاهز لتأمين عملك؟" : "Ready to Secure Your Business?");
  const sectionDescription = isEditing
    ? stripHtml(form.descriptionEn || form.descriptionAr)
    : stripHtml(securityModalSection?.description?.[locale] || "");
    ...
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="text-center text-white/60">
          {isArabic ? "جاري التحميل..." : "Loading Security Modal section..."}
        </div>
      </div>
    );
  }

  if (!securityModalSection && !isEditing) {
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="text-center text-white/60">
          {isArabic
            ? "لا يوجد قسم Security Modal للعرض"
            : "No Security Modal section found"}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">
          {isArabic ? "قسم Security Modal" : "Security Modal Section"}
        </h2>
        {!isEditing ? (
          <Button
      ...
      {/* Security Modal Display/Edit Section */}
      <div className="relative bg-gradient-to-b from-gray-900 to-black rounded-3xl border border-gray-200 overflow-hidden shadow-lg">
        {isEditing ? (
          ...
          /* View Mode - Similar to SecurityModal component */
