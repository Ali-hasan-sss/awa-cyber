...
export default function SecurityTechnologiesSection() {
  const { locale } = useLanguage();
  const isArabic = locale === "ar";
  const [securityTechnologiesSection, setSecurityTechnologiesSection] =
  ...
  useEffect(() => {
    loadSecurityTechnologiesSection();
  }, []);

  const loadSecurityTechnologiesSection = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getSections({ page: "home" });
      // Get the eighth section (index 7) sorted by order
      const sortedData = data.sort((a, b) => a.order - b.order);
      if (sortedData && sortedData.length > 7) {
        const section = sortedData[7];
        setSecurityTechnologiesSection(section);
        setForm({
      ...
      setError(err.message || "Failed to load Security Technologies section");
      console.error("Error loading Security Technologies section:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!securityTechnologiesSection) return;
    ...
    try {
      ...
      await updateSection(securityTechnologiesSection._id, payload);
      await loadSecurityTechnologiesSection();
      setIsEditing(false);
    } catch (err: any) {
      setError(err.message || "Failed to save Security Technologies section");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCancel = () => {
    if (securityTechnologiesSection) {
      setForm({
        titleEn: securityTechnologiesSection.title?.en || "",
        titleAr: securityTechnologiesSection.title?.ar || "",
        descriptionEn: securityTechnologiesSection.description?.en || "",
        descriptionAr: securityTechnologiesSection.description?.ar || "",
        features:
          securityTechnologiesSection.features &&
          securityTechnologiesSection.features.length > 0
            ? securityTechnologiesSection.features.map((feature) => ({
                icon: feature.icon || "",
                nameEn: feature.name?.en || "",
                nameAr: feature.name?.ar || "",
                descriptionEn: feature.description?.en || "",
  ...
  const sectionTitle = isEditing
    ...
    : securityTechnologiesSection?.title?.[locale] ||
      (locale === "ar" ? "تقنيات الأمان" : "Security Technologies");
  const sectionDescription = isEditing
    ? stripHtml(form.descriptionEn || form.descriptionAr)
    : stripHtml(securityTechnologiesSection?.description?.[locale] || "");
  ...
  if (!securityTechnologiesSection && !isEditing) {
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        <div className="text-center text-white/60">
          {isArabic
            ? "لا يوجد قسم تقنيات الأمان للعرض"
            : "No Security Technologies section found"}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">
          {isArabic ? "قسم تقنيات الأمان" : "Security Technologies Section"}
        </h2>
        {!isEditing ? (
          <Button
      ...
      {/* Security Technologies Display/Edit Section */}
        ...
        {isEditing ? (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Right Column - Technologies Management */}
                  ...
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-white/60">
                        {isArabic
                          ? "أضف التقنيات مع الأيقونة والاسم والوصف"
                          : "Add technologies with icon, name and description"}
                      </p>
                    </div>
                    <Button
                      type="button"
                    ...
                    <div className="mb-4 rounded-2xl border border-cyan-400/30 bg-cyan-500/10 p-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-semibold text-cyan-300">
                          {isArabic ? "إضافة تقنية" : "Add Technology"}
                        </h4>
                        <div className="flex gap-2">
                          <Button
                            type="button"
                  ...
                  <div className="space-y-3 max-h-[600px] overflow-y-auto">
                    {form.features.length === 0 && !draftFeature && (
                      <p className="text-sm text-white/60 text-center py-4">
                        {isArabic ? "لا توجد تقنيات" : "No technologies yet"}
                      </p>
                    )}
          ...
          /* View Mode - Similar to SecurityTechnologies component */
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="text-sm text-muted-foreground">
                            {tool.descriptionEn || tool.descriptionAr || ""}
                          </p>
                        </div>
                      );
                    })}
